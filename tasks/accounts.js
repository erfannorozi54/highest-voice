...
task("accounts", "Prints the list of accounts and their private keys", async (taskArgs, hre) => {
  const accountsConfig = hre.config.networks.hardhat.accounts;
  if (!accountsConfig || !accountsConfig.mnemonic) {
    console.log("Mnemonic not found in hardhat.config.js for the hardhat network.");
    return;
  }
  ...
  const masterNode = hre.ethers.HDNodeWallet.fromSeed(hre.ethers.Mnemonic.fromPhrase(accountsConfig.mnemonic).computeSeed());
